...
// 6. Verificar que UpdateItemAsync haya sido invocado exactamente una vez con los par�metros esperados.
//
// Notas:
// - Usamos xUnit para las pruebas y Moq para el mock del repositorio.
// - El test es as�ncrono y m�nimo: valida que el handler delega correctamente la actualizaci�n al repositorio.
...
namespace MiKompri.ShoppingList.Application.Tests.Commands.UpdateItemShoppingList
{
    public class UpdateItemShoopingListCommandHandlerTests
    {
        ...
        public async Task Handle_CallsUpdateItemAsync_AndReturnsTrue()
        {
            ...
            // Se asume que UpdateItemShoopingListCommand tiene propiedades p�blicas compatibles con las usadas en el handler.
        ...
        [Fact]
        public async Task Handle_Allows_Partial_Update_With_Nullable_Fields()
        {
            // Arrange
            var repoMock = new Mock<IPurchaseListRepository>(MockBehavior.Strict);
            repoMock
            ...
            var listId = Guid.NewGuid();
            var prodId = Guid.NewGuid();
            var request = new UpdateItemShoopingListCommand(listId, prodId, "Manzanas", null, null);
            ...
            // Assert
            Assert.True(result);
            repoMock.Verify(r => r.UpdateItemAsync(listId, prodId, "Manzanas", null, null), Times.Once);
            uowMock.Verify(u => u.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}