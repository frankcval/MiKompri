...
// 2. Probar operaciones b�sicas de la entidad:
...
// 3. Probar gesti�n de items:
//    - `AddItem` agrega correctamente el item.
//    - `UpdateItem` actualiza nombre, precio y cantidad; lanzar excepci�n si no existe.
//    - `DeleteItem` elimina el item; lanzar excepci�n si no existe.
//    - `MarkItemAsPurchased` marca como comprado; lanzar excepci�n si no existe.
...
// 6. Usar constructores p�blicos de `ListItem` y las propiedades expuestas para aserciones.
// 7. Incluir pruebas para casos de error (excepciones esperadas).
//
// Implementaci�n: crear clase de pruebas `PurchaseListTests` con m�todos `[Fact]` para cada caso.
...
namespace MiKompri.ShoppingList.Domain.Tests
{
    public class PurchaseListTests
    {
        ...
        public void UpdateDescription_ActualizaDescripcionYUpdatedAt()
        {
            ...
            list.UpdateDescription("Mi descripci�n");

            Assert.Equal("Mi descripci�n", list.Description);
            Assert.NotNull(list.UpdatedAt);
            Assert.True(list.UpdatedAt > before);
        }
        ...
        [Fact]
        public void AddItem_LanzaSiProductoDuplicado()
        {
            var owner = Guid.NewGuid();
            var productId = Guid.NewGuid();
            var list = new PurchaseList("Lista", owner);

            list.AddItem(new ListItem(productId, "Leche", 1.25m, 2));

            Assert.Throws<InvalidOperationException>(() =>
                list.AddItem(new ListItem(productId, "Leche Premium", 1.50m, 1)));
        }

        [Fact]
        public void Constructor_LanzaSiNombreEsInvalido()
        {
            var owner = Guid.NewGuid();

            Assert.Throws<InvalidOperationException>(() => new PurchaseList("   ", owner));
        }
        ...
        [Fact]
        public void MarkItemAsPurchased_EsIdempotente()
        {
            var owner = Guid.NewGuid();
            var list = new PurchaseList("Lista", owner);
            var item = new ListItem(Guid.NewGuid(), "Huevos", 1.75m, 12);
        ...
        public void Progress_ReflejaPorcentajeDeComprados()
        {
            ...
            // Se espera 50.00 (redondeado a 2 decimales seg�n implementaci�n)
