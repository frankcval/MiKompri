...
- [X] T007 Modificar `MiKompri.Users.Domain/Abstractions/Entity.cs`: añadir `public DateTime CreatedAt { get; protected set; }` y `public DateTime UpdatedAt { get; protected set; }`; todos los constructores de agregado que hereden deben inicializar ambas propiedades con `DateTime.UtcNow`. Crear `MiKompri.Users.Domain/Abstractions/ForbiddenOperationException.cs`: excepción `sealed` que hereda de `Exception` con un constructor `(string message)` — usada por el dominio para violaciones de matriz de roles (mapeada a HTTP 403 por el middleware); nunca para reglas de negocio (esas usan `InvalidOperationException` → 400) [C2]
- [X] T008 [P] Modificar `MiKompri.Users.Domain/Users/GroupRole.cs`: establecer `Owner = 1`, añadir `Admin = 2` (nuevo), cambiar `Member` a `Member = 3` (antes era 2 — seguro: EF usa `HasConversion<string>()`, la BD almacena el nombre del enum, no el valor entero)
- [X] T009 [P] [US1] Modificar `MiKompri.Users.Domain/Users/User.cs`: inicializar `CreatedAt`/`UpdatedAt` en el constructor existente; añadir método `public void SyncClaims(string? displayName, string? email)` que actualiza `DisplayName` si `displayName != null && displayName != DisplayName`, actualiza `Email` si `email != Email`, y marca `UpdatedAt = DateTime.UtcNow` solo cuando hubo al menos un cambio
- [X] T010 [P] [US5] Modificar `MiKompri.Users.Domain/Users/GroupMembership.cs`: añadir propiedad `public DateTime JoinedAt { get; private set; }`; asignar `JoinedAt = DateTime.UtcNow` en el método de fábrica `Create()`; inicializar `CreatedAt`/`UpdatedAt` heredados de `Entity`
- [X] T011 [US5] Modificar `MiKompri.Users.Domain/Users/Group.cs`: inicializar `CreatedAt`/`UpdatedAt` en constructor; añadir método `public GroupRole? GetMemberRole(Guid userId)` que retorna el rol del miembro o `null` si no existe; actualizar método `RemoveMember` con firma `public void RemoveMember(Guid targetUserId, GroupRole requestingRole)` aplicando las siguientes reglas **en este orden**: (a) si target no existe en `_memberships` → throw `KeyNotFoundException` (el target no es miembro); (b) si `target.Role == Owner && HasSingleOwner()` → throw `InvalidOperationException("No se puede eliminar al último propietario...")` — 400 [C1, FR-010]; (c) si `requestingRole == Admin && target.Role != Member` → throw `ForbiddenOperationException("Un Admin solo puede eliminar miembros con rol Member.")` — 403 [C2, FR-009]; actualizar `UpdatedAt` en `AddMember` y en `RemoveMember` tras la eliminación exitosa
...
- [ ] T019 [P] [US1] Crear `MiKompri.Users.Application/Commands/SyncProfile/SyncProfileCommand.cs` (record con `string IdentityProvider`, `string ExternalUserId`, `string? DisplayName`, `string? Email`) y `SyncProfileCommandHandler.cs` con la siguiente lógica: si no existe perfil → `new User(identityProvider, externalUserId, displayName, email)` donde `email` es `null` si ausente (no cadena vacía) + guardar + retornar `(UserId, Created: true)` [C4]; si ya existe → llamar `user.SyncClaims(displayName, email)` y guardar **solo si `UpdatedAt` cambió** (detectar mediante comparación antes/después o añadiendo retorno `bool` a `SyncClaims`) → retornar `(UserId, Created: false)` [C5]; el handler NO debe guardar si no hay cambios para mantener idempotencia [FR-002, FR-016]
...
- [ ] T021 [P] [US5] Crear `MiKompri.Users.Application/Commands/RemoveMemberFromGroup/RemoveMemberFromGroupCommand.cs` (record con `Guid GroupId`, `Guid TargetUserId`), `RemoveMemberFromGroupCommandHandler.cs` con la siguiente lógica: (a) cargar grupo por `GroupId`; si no existe → throw `ForbiddenOperationException` — no `KeyNotFoundException` — para cumplir SR-006 (evitar enumeración de grupos) [G1]; (b) `callerRole = group.GetMemberRole(currentUserId)`; si `callerRole` es null → throw `ForbiddenOperationException("El caller no es miembro del grupo.")` [SR-006]; si `callerRole == Member` → throw `ForbiddenOperationException` [FR-012]; (c) llamar `group.RemoveMember(targetUserId, callerRole.Value)` — el dominio aplica FR-009 y FR-010; (d) guardar; y `RemoveMemberFromGroupCommandValidator.cs` (`NotEmpty` en ambos IDs) [FR-009, FR-010]
...
- [ ] T024 [P] [US6] Crear `MiKompri.Users.Application/Queries/GetGroupMembers/GetGroupMembersQuery.cs` (record con `Guid GroupId`) y `GetGroupMembersQueryHandler.cs` con la siguiente lógica: (a) cargar grupo con `Include(g => g.Memberships)` por `GroupId`; si no existe → throw `ForbiddenOperationException` (no `KeyNotFoundException`) para cumplir SR-006 [G1, C2]; (b) `callerRole = group.GetMemberRole(currentUserId)`; si null → throw `ForbiddenOperationException("El caller no es miembro del grupo.")` [SR-006]; (c) cargar todos los perfiles de usuario en un solo roundtrip: query `_context.Users.Where(u => memberIds.Contains(u.Id)).ToDictionaryAsync(u => u.Id)` — NO usar un bucle de `GetByIdAsync` por cada miembro [G3 anti-N+1]; (d) mapear a `IReadOnlyCollection<GroupMemberDto>` con `DisplayName` y `JoinedAt` [FR-011]
- [ ] T025 [US5] Modificar `MiKompri.Users.Application/Commands/AddMemberToGroup/AddMemberToGroupCommandHandler.cs`: (a) cargar grupo por `GroupId`; si no existe → throw `ForbiddenOperationException` (SR-006, no `KeyNotFoundException`) [G1]; (b) `callerRole = group.GetMemberRole(currentUserId)`; si null o `Member` → throw `ForbiddenOperationException` [C2, FR-012]; (c) si `callerRole == Admin && request.Role == GroupRole.Admin` → throw `ForbiddenOperationException("Solo el Owner puede asignar rol Admin.")` [C2, FR-007]; (d) verificar que el usuario a agregar existe vía `IUserRepository.GetByIdAsync`; si no existe → throw `InvalidOperationException` (400, no 404 — se desconoce el recurso pero es un error del caller); (e) llamar `group.AddMember(request.MemberUserId, request.Role)` [FR-007]
...
- [ ] T028 Copiar `MiKompri.ShoppingList.Api/Middleware/ExceptionHandlingMiddleware.cs` a `MiKompri.Users.Api/Middleware/ExceptionHandlingMiddleware.cs` cambiando el namespace a `MiKompri.Users.Api.Middleware`; **ampliar el mapeo** añadiendo un case para `ForbiddenOperationException` → `StatusCodes.Status403Forbidden` antes del case de `InvalidOperationException` → 400 [C2]; verificar que el mapeo final es: `ValidationException` → 400, `InvalidOperationException` → 400, `ForbiddenOperationException` → 403, `KeyNotFoundException` → 404; todos incluyen `traceId` en el cuerpo de error
- [ ] T029 [P] Añadir paquete `Serilog.AspNetCore` 8.x a `MiKompri.Users.Api/MiKompri.Users.Api.csproj` si no está ya referenciado (verificar con `dotnet list MiKompri.Users.Api package`); **no crear ningún middleware manual de request logging**: Serilog lo provee a través de `app.UseSerilogRequestLogging()` que se añade en T032 [U3]
- [ ] T030 [US1] Crear `MiKompri.Users.Api/Middleware/UserProvisioningMiddleware.cs`: si `context.User.Identity?.IsAuthenticated != true` → `await _next(context); return`; leer claim `"sub"` — si vacío o nulo → escribir respuesta JSON genérica **con body** `{"status":401,"error":"Authentication failed.","traceId":"<Activity.Current.Id>"}` + `context.Response.StatusCode = 401` + `return` (SR-005: sin detalles internos; S1: con body consistente con FR-013/FR-014); leer claims `"name"` (nullable) y `"email"` (nullable); enviar `SyncProfileCommand` vía `ISender`; asignar `context.Items["UserId"] = result.UserId`; `await _next(context)` [FR-002, SR-005]
...
- [ ] T032 Reconstruir `MiKompri.Users.Api/Program.cs` completo con este orden de registro de servicios: (1) Serilog bootstrap; (2) `AddControllers()`; (3) `AddUsersApplication()`; (4) `AddUsersInfrastructure(configuration)` con connection string `"UsersPostgreSQL"`; (5) `AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(o => { o.Authority = config["Authentication:Authority"]; o.Audience = config["Authentication:Audience"]; o.TokenValidationParameters = new() { ValidateIssuer = true, ValidateAudience = true, ValidateLifetime = true, ValidateIssuerSigningKey = true }; })` [C3 — `ValidateIssuerSigningKey = true` es obligatorio para SR-001; con `Authority` configurado, ASP.NET Core descarga automáticamente las claves JWKS del IdP al arrancar]; (6) `AddSwaggerGen()` con `AddSecurityDefinition("Bearer", ...)` y `AddSecurityRequirement(...)`; (7) `AddHttpContextAccessor()`; (8) `AddScoped<ICurrentUserService, HttpCurrentUserService>()`; (9) `AddHealthChecks().AddDbContextCheck<UsersDbContext>()`; (10) **añadir validación de arranque**: si `config["Authentication:Authority"]` es null o vacía lanzar `InvalidOperationException("Authentication:Authority no puede estar vacía.")` para evitar despliegues con configuración incompleta [S2]; pipeline de middleware (orden crítico): `UseSerilogRequestLogging` → `UseGlobalExceptionHandling()` → `UseCors("MiKompriCors")` → `UseAuthentication()` → `UseAuthorization()` → `UseMiddleware<UserProvisioningMiddleware>()` → `MapControllers()` → `MapHealthChecks("/health")`; añadir auto-migrate en Development (`Database.Migrate()`)
...
- [ ] T034 [P] [US5] Crear `test/MiKompri.Users.Domain.Tests/GroupTests.cs`: (1) `Constructor_RegistersOwnerMembership`; (2) `AddMember_Duplicate_ThrowsInvalidOperationException`; (3) `RemoveMember_LastOwner_ThrowsInvalidOperationException` [C1 — último Owner → 400]; (4) `RemoveMember_OwnerWithMultipleOwners_Succeeds` [C1 — no el último Owner → ok]; (5) `RemoveMember_AdminRemovesMember_Succeeds`; (6) `RemoveMember_AdminRemovesAdmin_ThrowsForbiddenOperationException` [C2 → 403]; (7) `RemoveMember_AdminRemovesOwner_ThrowsForbiddenOperationException` [C2 → 403]; (8) `RemoveMember_MemberNotInGroup_ThrowsKeyNotFoundException`
...
- [ ] T039 [P] [US4] Verificar que existe `MiKompri.Users.Application/Commands/CreateGroup/CreateGroupCommandValidator.cs`; si no existe, crearlo con reglas `NotEmpty` y `MaximumLength(100)` en la propiedad `Name` del `CreateGroupCommand` [G2, FR-005]. Crear `test/MiKompri.Users.Application.Tests/Commands/CreateGroupCommandHandlerTests.cs`: (1) no autenticado → throws; (2) nombre válido → devuelve `GroupId`; (3) Owner registrado automáticamente en membresías; (4) nombre vacío → `ValidationException` [G2]; (5) nombre de 101 caracteres → `ValidationException` [G2]
...
- [ ] T041 [P] [US5] Crear `test/MiKompri.Users.Application.Tests/Commands/RemoveMemberFromGroupCommandHandlerTests.cs`: Owner elimina Member → ok; Owner elimina Admin → ok; Owner elimina Owner (no último) → ok; Admin elimina Member → ok; Admin elimina Admin → throws `ForbiddenOperationException` [C2]; Admin elimina Owner → throws `ForbiddenOperationException` [C2]; último Owner se intenta eliminar a sí mismo → throws `InvalidOperationException` [C1, FR-010]; grupo no existe → throws `ForbiddenOperationException` [G1, SR-006]; target no es miembro del grupo → `KeyNotFoundException`
...
- [ ] T044 Crear `test/MiKompri.Users.Api.Tests/CustomWebApplicationFactory.cs`: reemplazar opciones de `UsersDbContext` por EF InMemory; registrar `TestAuthHandler` como esquema Bearer; exponer método helper `CreateAuthenticatedClient(string sub, string? name = null, string? email = null)` [Patrón: `MiKompri.ShoppingList.Api.Tests/CustomWebApplicationFactory.cs`]. Crear `TestAuthHandler.cs` que genera `ClaimsIdentity` con los claims recibidos; cuando `sub` sea `string.Empty` o nulo el handler DEBE retornar `AuthenticateResult.Fail("No sub claim")` para que el pipeline produzca una respuesta real de 401 con body genérico [G4, S1]
- [ ] T045 [P] [US1] [US2] [US3] Crear `test/MiKompri.Users.Api.Tests/ProfileApiTests.cs`: (1) `GET /api/v1/users/me` sin auth → 401; (2) `GET /api/v1/users/me` con auth → 200 + perfil auto-provisionado; (3) segundo `GET` mismo token → 200, mismo `id`, sin duplicar; (4) `PUT /api/v1/users/me` con displayName válido → 200; (5) `PUT` con displayName vacío → 400 con `errors[0].field = "DisplayName"`; (6) `POST /api/v1/users/me/sync` primera llamada → 201; (7) segunda llamada mismo token → 200; **(8) `POST /api/v1/users/me/sync` con token sin claim `sub` (usar `CreateAuthenticatedClient(sub: string.Empty)`) → 401 con body JSON genérico** [G4, FR-002, SR-005]
- [ ] T046 [P] [US4] [US5] [US6] Crear `test/MiKompri.Users.Api.Tests/GroupsApiTests.cs`: (1) `POST /api/v1/groups` nombre válido → 201 + `Location` header; (2) `POST` nombre vacío → 400; (3) `GET /api/v1/groups` con grupos → 200 con `myRole`; (4) `GET /api/v1/groups` sin grupos → 200 array vacío; (5) `GET /{id}/members` como Owner → 200 con miembros; (6) `GET /{id}/members` sin membresía → 403; (7) Owner agrega Member → 201; (8) Admin intenta agregar Admin → 403; (9) Admin elimina Admin → 403; (10) duplicado → 400; (11) eliminar único Owner → 400; **(12) `GET /{id}/members` con `groupId` que no existe → 403 (mismo código que sin membresía, SR-006 anti-enumeración)** [G1]
...
- [ ] T057 Crear `.github/workflows/ci-mikompri-users.yml` con los siguientes pasos [G5, TP4, TP8]: (1) `dotnet restore MiKompri.sln`; (2) `dotnet build MiKompri.sln --configuration Release --no-restore`; (3) `dotnet test test/MiKompri.Users.Domain.Tests/... --configuration Release`; (4) `dotnet test test/MiKompri.Users.Application.Tests/... --configuration Release`; (5) `dotnet test test/MiKompri.Users.Api.Tests/... --configuration Release`; (6) `docker build -t mikompri.users.api -f MiKompri.Users.Api/Dockerfile .` — el workflow DEBE fallar si el build Docker falla [TP4]; trigger: `push` y `pull_request` en `003-users-authentication` y `main`
- [ ] T058 [P] Añadir en `test/MiKompri.Users.Api.Tests/ProfileApiTests.cs` un test de smoke de OpenAPI: `GET /swagger/v1/swagger.json` con cliente sin auth → `StatusCode == 200` y `Content-Type` contiene `application/json`; este test valida que el esquema OpenAPI se genera correctamente y no está roto [U1, TP7]
...
Phase 8: Tests:
  ...
  └── T044–T046 Integration tests → Phase 6 + Phase 7 completas
      └── T058 OpenAPI smoke test → dentro de T045/T046 (Phase 8)
T057 CI workflow → puede crearse en paralelo con Phase 9, después de que los proyectos de test existan (Phase 1)
```
...
- **Total de tareas**: 58 (T001–T058)
- **Tareas paralelas [P]**: 36
- **Distribución por fase**: Setup 6 · Dominio 5 · Infraestructura 4 · Auth 2 · Application 8 · Controllers 2 · Middleware 6 · Tests 13 · Docker 6 (T047–T050 + T057 CI + T058 OpenAPI) · Verificación 6
- **MVP scope**: T001–T011 + T016–T019 + T022 + T026 (parcial) + T028–T032 → US1 + US2 funcionando sin Docker
- **Gates de constitución**: TP4 cubierto por T047 (Dockerfile) + T057 (`docker build` en CI); TP7 cubierto por T058 (OpenAPI smoke); TP8 cubierto por T002–T004 + T034–T046
- **Sin modificaciones a ShoppingList**: ninguna tarea toca archivos de `MiKompri.ShoppingList.*` excepto T049 (corrección de typo en docker-compose.override.yml — mínimo indispensable)
- **Format validation**: todos los tasks siguen el formato `- [ ] TXXX [P?] [USx?] Descripción con ruta de archivo`
- **Correcciones del análisis aplicadas**: C1 C2 C3 C4 C5 G1 G2 G3 G4 G5 S1 S2 U1 U2 U3 Sc1 Sc2 A1 (18/18 hallazgos resueltos)
